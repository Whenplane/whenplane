<script lang="ts">
	...
	let userVote = $state(
		browser
			? (JSON.parse(localStorage.getItem('latenessVote') ?? '{"lastVote":0}') as UserVote)
			: { lastVote: 0 }
	);
...
>
	{#each page.data.liveStatus?.votes as option}
		...
		<button
			class="block w-full text-left background relative"
			formaction="?/vote&for={encodeURIComponent(option.name)}&k={vd}"
			class:passed
		>

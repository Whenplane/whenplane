<script lang="ts">
	...
	let {
		subTopics = typed<Timestamp[]>(),
		youtubeId = typed<string>(),
		floatplaneId = typed<string>(),
		floatplane = typed<boolean>(false),
		preShowLength = typed<number | undefined>()
	} = $props();
	...
	let expanded = $state(
		rest.length <= 1 ||
			rest.map((t) => '#timestamp-' + youtubeId + '.' + t.time).includes(page.url.hash)
	);
</script>

<script lang="ts">
	...
	let { shortResponse = typed<NotablePeopleShortResponse>() } = $props();
...
<a
	class="card border-2 p-2 border-amber-600! bg-amber-600/21! block relative"
...
>
	...
	<img
		src="/profiles/{shortResponse.name.toLowerCase()}.webp"
		class="inline-block placeholder-circle h-32"
		alt={shortResponse.name}
	/>

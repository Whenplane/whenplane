...
<div class="p-1">
	<div class="card limit mx-auto pb-4 pt-2 relative">
		...
		{#await records}
			<LoadingRecord>Earliest Show</LoadingRecord>
			<LoadingRecord>Most late show</LoadingRecord>
			<Accordion collapsible>
				<Accordion.Item value="a">
					...
					<Accordion.ItemContent>Loading...</Accordion.ItemContent>
				</Accordion.Item>
			</Accordion>
		{:then rec}
			...
			{#await rec.mostLate}
				<LoadingRecord>Most late show</LoadingRecord>
			{:then record}
				<Record {record} late={true}>Most late show</Record>
			{/await}
			<br />
			<Accordion class="px-4" collapsible onValueChange={fetchLatenesses}>
				<Accordion.Item value="b">
					...
					<Accordion.ItemContent>
						{#await rec.longestPreShow}
							<LoadingRecord>Longest pre-show</LoadingRecord>
						{:then record}
							<Record {record}>Longest pre-show</Record>
						{/await}
						{#await rec.longestShow}
							<LoadingRecord>Longest main show</LoadingRecord>
						{:then record}
							<Record {record}>Longest main show</Record>
						{/await}
						{#await rec.shortestPreShow}
							<LoadingRecord>Shortest pre-show</LoadingRecord>
						{:then record}
							<Record {record}>Shortest pre-show</Record>
						{/await}
						{#await rec.shortestShow}
							<LoadingRecord>Shortest main show</LoadingRecord>
						{:then record}
							<Record {record}>Shortest main show</Record>
						{/await}
						...
						{#await rec.showStreak}
							<LoadingRecord>
								Show Streak
								{#snippet description()}
									<span class="opacity-75 text-90 relative bottom-1"
										>Shows aired without missing a week</span
									>
								{/snippet}
							</LoadingRecord>
						{:then record}
						...
						<div class="opacity-75">
							...
							I'm ajgeiss0702 on floatplane and discord, or you can email me at
							<a href="mailto:aj@whenplane.com?subject=Record%2Fstat%20suggestion"
								>aj@whenplane.com</a
							>
						</div>
					</Accordion.ItemContent>

...
<script lang="ts">
	...
	let {
		...
		withThumbnail = typed<boolean>(false),
		lazyLoadThumbnail = typed<boolean>(false),
		onlyTimes = typed<boolean>(false),
		alternateStartTimes = typed<AlternateTimeRow[]>(),
		lazyLoadGroup = typed<any>(lazyLoadThumbnail ? Math.random() : -1)
	} = $props();
	...
	const thumbnail =
		findThumbnailsAt?.thumbnails?.maxres ??
	...
	const preShowStart = show.metadata.preShowStart
		? new Date(show.metadata.preShowStart)
		: show.metadata.preShowStart;
	const mainShowStart = show.metadata.mainShowStart
		? new Date(show.metadata.mainShowStart)
		: show.metadata.mainShowStart;
	const showEnd = show.metadata.showEnd ? new Date(show.metadata.showEnd) : show.metadata.showEnd;

	const showDate = getClosestWan(
		new Date(preShowStart ?? mainShowStart ?? showEnd ?? snippet?.publishedAt),
		alternateStartTimes
	);

	const preShowLength =
		preShowStart && mainShowStart
			? getTimeUntil(mainShowStart, preShowStart.getTime()).string
			: null;
	const mainShowLength =
		mainShowStart && showEnd
			? getTimeUntil(showEnd, mainShowStart.getTime()).string
			: show.metadata?.mainShowLength
				? timeString(show.metadata.mainShowLength)
				: null;
				...
				{#if lazyLoadThumbnail}
					<div class="thumbnail-space relative">
						...
						{#if isLazyLoadTrigger && !$groupStore.thumbnailLoaded}
							<LargerLazyLoad
								on:visible={() => {
									console.debug('Showing group', lazyLoadGroup);
									$groupStore.thumbnailLoaded = true;
								}}
							/>
						{/if}

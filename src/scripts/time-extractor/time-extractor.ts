...
export async function download(id: string) {
  ...
  return new Promise<object>((resolve, reject) => {
    const dl = execCallback(`youtube-dl --exec "ffmpeg -init_hw_device vaapi=foo:/dev/dri/renderD128 -hwaccel vaapi -hwaccel_device foo -i {} -filter:v \\"crop=150:150:in_w-150:in_h-150\\" tmp.mp4 && mv tmp.mp4 vid.mp4 && rm {}" ${id}`, {

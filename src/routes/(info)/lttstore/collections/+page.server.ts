...
export const load = (async ({platform}) => {
  ...
  const collections = await retryD1(() =>
    db.prepare("select id,title,handle,published_at,updated_at,image,reportedCount,json_array_length(products) as observedCount,available from collections order by updated_at desc")

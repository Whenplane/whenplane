...
export const GET = (async ({platform, url}) => {
  ...
  const response = await retryD1(() =>
    (cursor == null)
      ? db
        .prepare("select * from collection_changes order by timestamp desc, change_id desc limit ?")
        .bind(
          perPage + 1, // pre-fetch 1 extra to see if there is more than the current page
        )
        .all<ChangeHistoryTableRow & {change_id: number}>()
      : db
        .prepare("select * from collection_changes where (timestamp, change_id) < (?, ?) order by timestamp desc, change_id desc limit ?")
        .bind(
          cursorTimestamp,
          cursorRow,
          perPage + 1, // pre-fetch 1 extra to see if there is more than the current page

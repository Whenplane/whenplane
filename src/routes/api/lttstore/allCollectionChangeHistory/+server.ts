...
export const GET = (async ({platform, url}) => {
  ...
  const results = await retryD1(() =>
    (cursor == null)
      ? db
        .prepare("select rowid,* from collection_changes order by timestamp desc, rowid desc limit ?")
        .bind(
        ...
        .then(r => r.results)
      : db
        .prepare("select rowid,* from collection_changes where (timestamp, rowid) < (?, ?) order by timestamp desc, rowid desc limit ?")
        .bind(
          cursorTimestamp,
          cursorRow,
          perPage + 1, // pre-fetch 1 extra to see if there is more than the current page
